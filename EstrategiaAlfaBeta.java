        ...
        if (esCapaMAX(capa)) {
            ...
            for (col = 0; col < Tablero.NCOLUMNAS; col++) {
                if (movimientosPosibles[col]) {
                    ...
                    nuevoTablero.anadirFicha(col, jugador);
                    nuevoTablero.obtenerGanador();
                    valorSucesor = ALFABETA(nuevoTablero, Jugador.alternarJugador(jugador), capa + 1, alfa_actual, beta);
                    nuevoTablero = null;
            ...
            for (col = 0; col < Tablero.NCOLUMNAS; col++) {
                if (movimientosPosibles[col]) {
                    ...
                    nuevoTablero.anadirFicha(col, jugador);
                    nuevoTablero.obtenerGanador();
                    valorSucesor = ALFABETA(nuevoTablero, Jugador.alternarJugador(jugador), capa + 1, alfa, beta_actual);
                    nuevoTablero = null;
